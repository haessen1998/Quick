    ...
    <div ref={containerRef} className="mermaid-preview my-4 w-full min-w-0 max-w-full overflow-hidden rounded-xl border bg-background/95 shadow-sm fullscreen:m-0 fullscreen:h-screen fullscreen:w-screen fullscreen:max-w-none fullscreen:rounded-none fullscreen:border-0">
      <div className="flex h-11 min-w-0 items-center justify-end gap-1 border-b bg-muted/25 px-2 sm:px-3">
        ...
        <div className="flex shrink-0 items-center gap-1">
          <Button type="button" variant="ghost" size="icon-xs" disabled={zoom <= 0.5} onClick={() => setZoom((value) => Math.max(0.5, value - 0.25))} aria-label={uiText("缩小图表")}><Minus /></Button>
          <button type="button" className="app-interactive min-w-12 rounded px-1 text-center text-[11px] text-muted-foreground hover:text-foreground" onClick={resetView} title={uiText("恢复 100%")}>{Math.round(zoom * 100)}%</button>
          <Button type="button" variant="ghost" size="icon-xs" disabled={zoom >= 2.5} onClick={() => setZoom((value) => Math.min(2.5, value + 0.25))} aria-label={uiText("放大图表")}><Plus /></Button>
          <Button type="button" variant="ghost" size="icon-xs" onClick={resetView} aria-label={uiText("重置缩放与位置")}><RotateCcw /></Button>
          <Button type="button" variant="ghost" size="icon-xs" onClick={() => void copySVG()} aria-label={uiText("复制 Mermaid SVG")}>{copied ? <Check /> : <Copy />}</Button>
          <Button type="button" variant="ghost" size="icon-xs" onClick={() => void toggleFullscreen()} aria-label={fullscreen ? uiText("退出全屏") : uiText("全屏预览")}><Maximize2 /></Button>
        </div>
      </div>
      <div ref={viewportRef} className={cn("mermaid-canvas h-[28rem] overflow-auto p-5 sm:p-7", panning ? "cursor-grabbing select-none" : "cursor-grab")} style={{ touchAction: "none" }} onPointerDown={startPan} onPointerMove={movePan} onPointerUp={stopPan} onPointerCancel={stopPan} aria-label={uiText("Mermaid 可拖动画布")} title={uiText("按住并拖动查看画布，滚动条仍可使用")}>
