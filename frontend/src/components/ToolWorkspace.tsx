  ...
  return (
    <section className="page-shell @container/workspace">
      <div className="mx-auto w-full max-w-7xl">
        ...
        <div className="grid items-start gap-4 @3xl/workspace:grid-cols-[12rem_minmax(0,1fr)]">
          ...
          <div className="min-w-0 rounded-xl border bg-card text-card-foreground shadow-sm">
            <div className="flex flex-wrap items-center gap-3 border-b p-4">
              <select
                className="h-9 w-full min-w-0 rounded-lg border border-input bg-background px-3 text-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 @3xl/workspace:hidden"
                aria-label={uiText("选择工具")}
                value={activeTool.id}
                onChange={(event) => {
                  const tool = tools.find((t) => t.id === event.target.value)
              ...
              <div className="flex flex-wrap items-center gap-2">
                {activeToolOptions?.[activeTool.id]}
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => {
                    setInput(activeTool.sample)
                    setOutput("")
                    setError("")
                  }}
                >
            ...
            <InputPreflight compact identity={activeTool.id} format={activeTool.id.split("-")[0]} input={input} />
            {error && (
            ...
            <div className="grid @4xl/workspace:grid-cols-2">
              <div className="min-w-0 border-b @4xl/workspace:border-r @4xl/workspace:border-b-0">
                <div className="flex h-10 items-center justify-between border-b px-4 text-xs text-muted-foreground">
                  <span>{t("输入")}</span>
                  <span>
                    {input.length} {language === "en-US" ? "characters" : uiText("字符")}
                  </span>
                </div>
                <CodeEditor
                  aria-label={t("输入")}
