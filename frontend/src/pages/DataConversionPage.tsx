    ...
    <section className="page-shell">
      <div className="mx-auto w-full max-w-7xl">
        ...
        <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
          ...
          <div className="grid lg:grid-cols-2"><label className="border-b lg:border-r lg:border-b-0"><div className="flex h-10 items-center justify-between border-b px-4 text-xs text-muted-foreground"><span>{t("输入")} · {t(module.sources.find((option) => option.id === source)?.label ?? "")}</span><span>{t(`${input.length} 字符`)}</span></div><CodeEditor className="app-interactive h-[28rem] w-full resize-none overflow-auto bg-transparent p-4 font-mono text-sm leading-6 outline-none" error={error} value={input} onChange={(event) => setInput(event.target.value)} spellCheck={false} /></label><div><div className="flex h-10 items-center justify-between border-b px-4 text-xs text-muted-foreground"><span>{t("输出")} · {t(module.targets.find((option) => option.id === target)?.label ?? "")}</span><Button variant="ghost" size="xs" disabled={!output} onClick={async () => { await writeClipboard(output); toast.success(t("已复制")) }}><Copy />{t("复制")}</Button></div><CodeEditor className="app-interactive h-[28rem] w-full resize-none overflow-auto bg-muted/20 p-4 font-mono text-sm leading-6 outline-none" readOnly value={output} placeholder={t("转换结果会显示在这里…")} /></div></div>
        </div>
      </div>
    </section>
  )
