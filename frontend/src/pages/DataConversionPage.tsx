...
export default function DataConversionPage() {
 const { t, moduleId, module, source, target, chooseTarget, sample, sampleRevision, loadSample, input, setInput, output, error, chooseModule, chooseSource, run, swappable, swap } = useDataConversionPageViewModel()
return (
    <section className="page-shell">
      <div className="mx-auto w-full max-w-7xl">
        ...
        <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
          <div className="grid gap-5 border-b p-4 lg:grid-cols-2"><RadioGroup legend={uiText("来源")} name={`${module.id}-source`} options={module.sources} value={source} disabledValue={module.sources.length > 1 ? target : undefined} onChange={chooseSource} /><RadioGroup legend={uiText("目标")} name={`${module.id}-target`} options={module.targets} value={target} disabledValue={module.targets.some((option) => option.id === source) ? source : undefined} onChange={chooseTarget} /></div>
          <div className="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3"><div><div className="text-sm font-medium">{t(module.label)}</div><div className="text-xs text-muted-foreground">{t(module.description)}</div></div><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={sample === undefined} onClick={loadSample}>{t("载入示例")}</Button><Button variant="outline" disabled={!swappable} onClick={swap}><ArrowLeftRight />{t("交换")}</Button><Button onClick={run} disabled={!input}><Play />{t("执行转换")}</Button></div></div>
          <InputPreflight identity={`${moduleId}/${source}`} resetKey={sampleRevision} format={source} input={input} />
