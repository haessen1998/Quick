...
export default function ValidationPage() {
  ...
  const selectMode = (next: Mode) => {
    ...
    if (next === "regex") { setExpression("(?<protocol>https?)://(?<host>[^/\\s]+)"); setInput("访问 https://github.com/haessen1998/Quick 或 http://localhost:9345"); setFlags("gi") }
  }
    ...
    try {
      const result = mode === "jsonpath"
        ...
        : mode === "xpath"
          ...
          : (() => {
              const uniqueFlags = Array.from(new Set(flags.replace(/[^dgimsuvy]/g, "").split(""))).join("")
              const regex = new RegExp(expression, uniqueFlags.includes("g") ? uniqueFlags : `${uniqueFlags}g`)
              const matches = Array.from(input.matchAll(regex), (match) => ({ value: match[0], index: match.index, groups: match.groups ?? {}, captures: match.slice(1) }))
        ...
        <div className="rounded-xl border bg-card shadow-sm">
          <div className="grid gap-3 border-b p-4 md:grid-cols-[1fr_auto]">
            <div className="grid gap-3 sm:grid-cols-[1fr_7rem]">
              ...
              {mode === "regex" ? <input className={inputClass} value={flags} onChange={(event) => setFlags(event.target.value)} placeholder="flags" /> : <div />}
            </div>
