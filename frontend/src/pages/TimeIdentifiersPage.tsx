...
function convertDuration(value: string, unit: DurationUnit, language: AppLanguage = "zh-CN") {
  ...
  if (!Number.isFinite(converted) || Math.abs(converted) > Number.MAX_SAFE_INTEGER) throw new Error(language === "en-US" ? "Duration exceeds the safe calculation range" : "时长超出安全计算范围")
  ...
  const formatted = language === "en-US"
...
function parseDuration(value: string, language: AppLanguage = "zh-CN") {
  ...
  if (colon) milliseconds = (Number(colon[2]) * 3600 + Number(colon[3]) * 60 + Number(colon[4] ?? 0)) * 1000 * (colon[1] === "-" ? -1 : 1)
    ...
    if (!matches.length || normalized.replace(/([+-]?\d+(?:\.\d+)?)\s*(ms|d|h|m|s)/g, "").trim()) throw new Error(language === "en-US" ? "Unrecognized duration; use 1d 2h 3m 4s 500ms or HH:MM:SS" : "无法识别时间段；支持 1d 2h 3m 4s 500ms 或 HH:MM:SS")
  ...
  if (!Number.isFinite(milliseconds) || Math.abs(milliseconds) > Number.MAX_SAFE_INTEGER) throw new Error(language === "en-US" ? "Duration exceeds the safe calculation range" : "时间段超出安全计算范围")
...
export default function TimeIdentifiersPage() {
  ...
  const dateToTimestamp = useMemo(() => {
    ...
    return language === "en-US" ? `Seconds: ${Math.floor(value.toMillis() / 1000)}\nMilliseconds: ${value.toMillis()}` : `秒：${Math.floor(value.toMillis() / 1000)}\n毫秒：${value.toMillis()}`
  ...
  const durationResult = useMemo(() => {
    try {
      ...
      return language === "en-US" ? `${result.formatted}\nTotal: ${result.totalMilliseconds} ms` : `${result.formatted}\n总计：${result.totalMilliseconds} 毫秒`
    } catch (error) {
  ...
  const timezoneComparisons = useMemo(() => {
    const value = DateTime.fromISO(dateInput, { zone: sourceZone })
    return comparisonZones.map((zone) => ({ zone, value: value.isValid ? value.setZone(zone).toFormat("yyyy-LL-dd HH:mm:ss ZZZZ") : t("无效时间") }))
