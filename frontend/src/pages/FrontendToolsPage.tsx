...
export default function FrontendToolsPage() {
  ...
  return <section className="page-shell"><div className="mx-auto w-full max-w-7xl"><div className="mb-6"><div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground"><Sparkles className="size-4" />开发工具</div><h1 className="text-3xl font-semibold tracking-tight">颜色与前端</h1><p className="mt-2 text-sm text-muted-foreground">颜色格式、对比度、渐变、阴影、CSS 单位与 SVG Data URL，全部在本地计算。</p></div><div className="grid gap-4 xl:grid-cols-2">
    ...
    <article className="rounded-xl border bg-card p-5 shadow-sm"><div className="flex items-center gap-2 font-medium"><SunMedium className="size-4" />对比度与渐变</div><div className="mt-4 grid grid-cols-2 gap-3"><input className={inputClass} value={foreground} onChange={(event) => setForeground(event.target.value)} /><input className={inputClass} value={background} onChange={(event) => setBackground(event.target.value)} /></div><div className="mt-3 rounded-lg border p-6 text-center font-medium" style={{ color: foreground, background }}>Quick Contrast · {contrast.toFixed(2)}:1</div><div className="mt-2 text-xs text-muted-foreground">普通文本 AA：{contrast >= 4.5 ? "通过" : "未通过"} · 大文本 AA：{contrast >= 3 ? "通过" : "未通过"}</div><div className="mt-4 h-20 rounded-lg border" style={{ background: gradient }} /><div className="mt-2 flex items-center gap-2"><input className={inputClass} type="number" value={angle} onChange={(event) => setAngle(Number(event.target.value))} /><Button variant="outline" onClick={() => copy(`background: ${gradient};`)}><Copy />CSS</Button></div></article>
