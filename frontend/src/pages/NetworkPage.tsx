...
export default function NetworkPage({ proxy }: { proxy: ProxySettings }) {
  ...
  const [pendingTermination, setPendingTermination] = useState<ProcessInfo | null>(null)
  ...
  const terminateProcess = async (process: ProcessInfo) => {
    ...
    try {
      ...
      if (!result.success) throw new Error(result.output)
      setPendingTermination(null); toast.success(`已关闭 ${process.name}`); await findProcesses()
    } catch (caught) { const message = caught instanceof Error ? caught.message : String(caught); toast.error("关闭进程失败", { description: message }) } finally { setTerminatingPID(null) }
  }
      ...
      <div className="mx-auto w-full max-w-7xl">
        ...
        <Dialog open={Boolean(pendingTermination)} onOpenChange={(open) => { if (!open && terminatingPID === null) setPendingTermination(null) }}>
          <DialogContent className="max-w-md">
            <DialogHeader><DialogTitle>确认强制关闭进程</DialogTitle><DialogDescription>此操作可能导致未保存的数据丢失，请确认目标进程无误。</DialogDescription></DialogHeader>
            ...
            <DialogFooter><DialogClose asChild><Button variant="outline" disabled={terminatingPID !== null}>取消</Button></DialogClose><Button variant="destructive" disabled={!pendingTermination || terminatingPID !== null} onClick={() => pendingTermination && void terminateProcess(pendingTermination)}><Power />{terminatingPID !== null ? "正在关闭…" : "强制关闭"}</Button></DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
    </section>
