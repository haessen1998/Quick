...
import { ToolWorkspaceModelProvider, type TextTool } from "@/models/ToolWorkspaceModel"
...
async function minifyHtmlText(input: string) {
  ...
  return minify(input, { collapseWhitespace: true, conservativeCollapse: true, removeComments: true, minifyCSS: false, minifyJS: false })
}
  ...
  return [
    {
      ...
      group: "JSON",
      label: "JSON 格式化",
      description: formatEmbeddedJSON ? "校验 JSON，并递归展开字符串中的 JSON 对象或数组" : "校验 JSON 并使用两个空格缩进",
      icon: FileJson,
      sample: jsonSample,
      run: (input) => formatJSON(input, 2, formatEmbeddedJSON),
    },
    {
      id: "json-minify",
      group: "JSON",
      ...
      icon: Braces,
      sample: JSON.stringify(JSON.parse(jsonSample), null, 2),
      run: (input) => formatJSON(input, 0),
    },
    {
      id: "yaml-format",
      group: "YAML",
    ...
    },
    {
      ...
      group: "XML",
      label: "XML 格式化",
      description: "校验 XML 并整理元素缩进",
      icon: FileCode2,
      sample: xmlSample,
      run: (input) => buildXml(input, true),
    },
    {
      id: "xml-minify",
      group: "XML",
      label: "XML 压缩",
      description: "移除 XML 元素间不必要的空白",
      icon: FileCode2,
      sample: buildXml(xmlSample, true),
      run: (input) => buildXml(input, false),
    },
    {
      id: "html-format",
      group: "HTML",
    ...
    },
    {
      ...
      group: "HTML",
      label: "HTML 压缩",
      description: "移除注释与标签间空白，不破坏文本内容",
      icon: Code2,
      sample: htmlSample,
      run: minifyHtmlText,
    },
    {
      ...
      group: "CSS",
      label: "CSS 格式化",
      description: "使用 Prettier 整理 CSS 规则",
      icon: FileCode2,
      sample: cssSample,
      run: formatCss,
    },
    ...
    },
    {
      ...
      description: "使用 Babel 解析并格式化 JavaScript",
      icon: Code2,
      sample: jsSample,
      run: formatJavaScript,
    },
    {
      id: "javascript-minify",
      group: "JavaScript",
      label: "JavaScript 压缩",
      description: "使用 Terser 压缩 JavaScript",
      icon: Code2,
      sample: jsSample,
      run: async (input) => {
...
export function FormatterModelProvider({ children }: { children: ReactNode }) {
  const [formatEmbeddedJSON, setFormatEmbeddedJSON] = useDraftState("formatter", "expandEmbeddedJSON", false)
  const tools = useMemo(() => createTools(formatEmbeddedJSON), [formatEmbeddedJSON])
  return (
    <ToolWorkspaceModelProvider
      title={uiText("字符串格式化")}
      description={uiText("格式化、校验与压缩 JSON、YAML、XML、HTML、CSS 和 JavaScript。")}
      tools={tools}
