...
try {
  ...
  code(async (page) => {
    ...
    try {
      ...
      await schema.fill('{"type":"object","properties":{"id":{"const":9007199254740993}}}')
      await page.getByRole("button", {name:"格式化 JSON Schema", exact:true}).click()
      const formatted = await schema.inputValue()
      if (!formatted.includes('\n  "type"') || !formatted.includes('9007199254740993')) throw Error("Manual schema format failed or lost precision")
      await schema.fill("{broken")
      await page.getByRole("button", {name:"格式化 JSON Schema", exact:true}).click()
      await page.getByText("JSON Schema 格式化失败，已保留原内容。可请小Q修复。", {exact:true}).waitFor()
      if (await schema.inputValue() !== "{broken") throw Error("Manual format lost invalid input")
  ...
  code(async page => {
    ...
    const latest = page.getByRole("dialog").locator("article").first()
    if (await latest.getByRole("button",{name:"发送到 JSON 格式化",exact:true}).count() || await latest.getByRole("button",{name:"JSON → YAML",exact:true}).count()) throw Error("Plain text routed as JSON")
    await latest.getByRole("button",{name:"复制结果",exact:true}).waitFor()
